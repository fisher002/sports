  ...
  <div class="game-list">
    <van-pull-refresh v-model="isLoading" success-text="刷新成功" @refresh="onRefresh">
      <div class="data-list" v-if="data.list && data.list.length > 0">
        ...
        <div v-if="edData" style="width: 100%">
          <div class="sort-title">已结束的赛事</div>
          <div class="list-item" v-for="item in edData" :key="item.id" @click="toDeatil(item.id)">
            <div class="item-title">{{item.schoolName}}</div>
            <div class="item-content">
              <div class="item-top font-size14">
                <div>名称：{{item.compateEventName}}</div>
          ...
          <van-empty v-if="willData.length <= 0" description="暂无已结束的比赛" />
        </div>
        ...
        res => {
          ...
          if (res.data.code == 10000) {
            if (type === "0") {
              ...
              this.edData.push(...this.sortDatae(res.data.data.list));
              return;
            }
            this.willData = this.sortDataw(res.data.data.list);
            this.ingData = this.sortDatai(res.data.data.list);
            this.edData = this.sortDatae(res.data.data.list);
