  ...
  <div>
    <van-tabs v-model="activeName" @change="resultBack">
      <van-tab title="赛事详情" name="a">
        ...
        <div style="margin: 16px 10px;" v-if="data.signUpStatus == 'Y'">
          <van-button
            :loading="loading"
            size="large"
        ...
        <div v-if="data.signUpStatus == 'N'">
          <van-button size="large" type="info" :disabled="true">报名已截止</van-button>
        </div>
      </van-tab>
      <van-tab title="已报名同学" name="b">
        <div v-if="classMates.list && classMates.list.length > 0" class="class-box">
          <div class="class-mate" v-for="item in classMates.list" :key="item.id">
            <div class="mate-name">{{item.studentName}}</div>
            <div class="mate-name mate-time">{{item.studentGroup == 'referee' ? '裁判' : '运动员'}}</div>
            <div class="mate-name mate-time">{{formatDate(item.createDate)}}</div>
          </div>
          <div
            class="class-mates"
            v-for="n in ((classMates.list.length % 2) == 0 ? 0 : 1)"
            :key="n"
          ></div>
        </div>
