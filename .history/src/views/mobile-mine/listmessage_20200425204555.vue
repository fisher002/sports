<template>
  <div>
          ...
          <div class="list-item" v-for="item in data.list" :key="item.id" @click="toAppeal(item)">
            ...
            <div class="item-content">
              <div class="item-top font-size14">
                ...
                <div class="color-g">点击可查看</div>
              </div>
              <div class="item-top font-size13 color-g">
                <div>备注：{{item.messageContent.substring(0,25)}}...</div>
              </div>
            </div>
          </div>
    ...
    >
      <van-form>
        <van-field
          v-model="messages.schoolCompateName"
          label="申述项目"
          clearable
          readonly
          label-align="right"
        />
        <van-field v-model="messages.studentName" label="名字" clearable readonly label-align="right" />
        <van-field
          v-model="messages.appealContent"
          label="申述内容"
          placeholder="请填写申述内容"
          type="textarea"
          maxlength="250"
