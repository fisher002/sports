<template>
  <div>
    <van-pull-refresh v-model="isLoading" success-text="刷新成功" @refresh="onRefresh" v-if="data">
      <div class="item-box">
        <div class="drop-menu">
          <van-dropdown-menu style="width:30%">
            <van-dropdown-item v-model="value" :options="events" @change="getValue" />
          </van-dropdown-menu>
          <van-search
            show-action
            style="width:70%"
            v-model="params.compateEventName"
            placeholder="请输入比赛名称"
          >
            <template #action>
              <div @click="onSearch">搜索</div>
            </template>
          </van-search>
        </div>
        <div class="item-list" v-if="data.list && data.list.length > 0">
          <div class="list-left">
            <van-sidebar v-model="activeKey">
              <van-sidebar-item
                v-for="item in data.list"
                :key="item.id"
                @click="getRule(item.rule)"
                :title="item.compateEventName"
              />
            </van-sidebar>
          </div>
          <div class="list-right" v-html="text ? text : data.list[0].rule"></div>
        </div>
        <van-empty style="width:100%" v-else description="暂无数据" />
        <div class="load-btn" v-show="data.hasNextPage">
          <van-button
            class="load-btn"
            @click="toLoadMore"
            :loading="loading"
            type="info"
            loading-text="加载中..."
          >点击加载更多</van-button>
        </div>
      </div>
    </van-pull-refresh>
    <van-empty v-else description="暂无数据，请先登录" />
  </div>
</template>
<script>
import { Toast } from "vant";
import api from "./itemUrl";
import utils from "@/utils/comUtils";

export default {
  data() {
    return {
      loading: false,
      isLoading: false,
      activeKey: 0,
      events: [],
      value: 0,
      data: "",
      text: "",
      pageNum: 1,
      params: {
        compateEventParentId: "",
        compateEventName: "",
        schoolId: ""
      }
    };
  },
  created() {
    if (utils.checkLogin()) {
      this.params.schoolId = sessionStorage.getItem("schoolId");
      this.getEventList();
    }
  },
  methods: {
    /**获取类型数据 */
    getEventList() {
      this.events.push({ text: "默认", value: 0 });
      api.getEventList({}).then(res => {
        if (res.data.code == 10000) {
          res.data.data.forEach(e => {
            if (e.parentId == null || e.parentId == "") {
              let demo = {
                text: e.compateEventName,
                value: e.id
              };
              this.events.push(demo);
            }
          });
          this.params.compateEventParentId = `${this.events[1].value}`;
          this.getSchoolCompatePageList();
        }
      });
    },
    /**获取比赛项目信息 */
    getSchoolCompatePageList(type) {
      let params = utils.checkParams(this.params);
      api.getSchoolCompatePageList(params, this.pageNum).then(
        res => {
          this.isLoading = false;
          this.loading = !this.loading;
          if (res.data.code == 10000) {
            if (type === "0") {
              this.data.list.push(...res.data.data.list);
              return;
            }
            this.data = res.data.data;
            return;
          }
          Toast(res.data.msg);
        },
        res => {
          this.isLoading = false;
          Toast(res.data.msg);
          this.loading = !this.loading;
        }
      );
    },
    /**加载更多 */
    toLoadMore() {
      this.loading = !this.loading;
      if (this.pageNum == this.data.pages) {
        return;
      }
      if (this.data.hasNextPage) {
        this.pageNum++;
        this.getSchoolCompatePageList("0");
      }
    },
    /**搜索 */
    onSearch() {
      this.getSchoolCompatePageList();
    },
    /**项目规则 */
    getRule(val) {
      this.text = val;
    },
    /**下拉刷新 */
    onRefresh() {
      this.getEventList();
    },
    /**下拉回调 */
    getValue(val) {
      if (val > 0) {
        this.params.compateEventParentId = `${val}`;
        this.getSchoolCompatePageList();
      }
    }
  }
};
</script>
<style lang="scss" scoped>
.item-box {
  width: 100%;
  display: flex;
  flex-wrap: wrap;
  padding: 5px 0;
  .drop-menu {
    display: flex;
    width: 100%;
  }
  .load-btn {
    width: 100%;
    height: 20px;
    line-height: 18px !important;
  }
  .item-list {
    padding: 5px 3px;
    width: 100%;
    display: flex;
    .list-left {
      width: 25%;
      padding: 0 3px;
    }
    .list-right {
      display: flex;
      justify-content: center;
      width: 70%;
      border: #d8caca 1px solid;
      border-radius: 5px;
      padding: 4px;
    }
  }
}
</style>