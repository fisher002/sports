          ...
          >
            <div class="class-mate" v-for="item in data.list" :key="item.id">
              <div class="mate-name">
                ...
                <van-checkbox :name="item.studentId"></van-checkbox>
              </div>
            </div>
          </van-checkbox-group>
...
export default {
  ...
  methods: {
    /**获取报名同学 */
    getStudentList(type) {
      this.param.schoolId = sessionStorage.getItem("schoolId");
      api.getCompateStudentPageList(this.param, this.pageNum).then(
        res => {
