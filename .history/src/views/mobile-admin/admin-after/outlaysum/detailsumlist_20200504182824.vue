        ...
        <div class="list-item" v-for="item in data.list" :key="item.id" @click="toDeatil(item)">
          ...
          <div class="item-content">
            <div class="item-top font-size14">
              <div>经费名称：{{item.outlayName}}</div>
              <div>费用：{{item.outlayMoney}}/元</div>
            </div>
  ...
  methods: {
    ...
    checkLogin() {
      if (utils.checkLogin()) {
        ...
        this.getOutPlayPageList();
      }
    },
    /**获取比赛项目信息 */
    getOutPlayPageList(type) {
      let params = utils.checkParams(this.params);
      api.getOutPlayPageList(params, this.pageNum).then(
        res => {
