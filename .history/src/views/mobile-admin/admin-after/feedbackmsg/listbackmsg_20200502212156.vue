<template>
  <div class="game-list">
    ...
    <van-dialog v-model="showDig" title="详情" show-cancel-button :showConfirmButton="false">
      <van-form @submit="onSubmit">
        <van-field v-model="message.compateName" label="项目" clearable readonly label-align="right" />
...
export default {
  ...
  methods: {
    ...
    onSubmit() {
      ...
      let demo = { equipmentId: this.message.id, status: "yes" };
      api.editMessage(demo).then(
        res => {
          this.scloading = !this.scloading;
          if (res.data.code == 10000) {
            this.showDig = false;
