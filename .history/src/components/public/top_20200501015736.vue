  ...
  <div>
    ...
    <van-popup v-model="showBtn" position="right" :style="{ height: '100%' }">
      <div class="side-btn">
        ...
        <van-button v-if="isLogin == false && isAdminLogin == false" block style="color:#1989fa" @click="toLogin">立即登录</van-button>
        <van-button v-if="isLogin == true || isAdminLogin == true" block style="color:#1989fa" @click="outLogin">退出登录</van-button>
...
import apiV1 from "@/views/mobile-admin/admin-login/loginUrl";
export default {
  inject: ["reload"],
  name: "top",
  data() {
  ...
  methods: {
    onClickLeft() {
      if (sessionStorage.getItem("isLogin") == "true" && this.noBack()) {
        return;
      }
